    ...
    <main className="min-h-screen bg-[#0f0e0c] px-6 py-16">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[#666] max-w-md mx-auto">Start free. No credit card. Barış will tell you if it's worth it — and he doesn't lie.</p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {plans.map((plan) => (
            <div key={plan.name} className={`rounded-2xl p-6 border ${plan.highlight ? "bg-[#1f1410] border-[#c84b2f]" : "bg-[#1a1917] border-[#2a2926]"}`}>
              {plan.highlight && (
                <span className="inline-block bg-[#c84b2f] text-white text-xs px-3 py-1 rounded-full mb-4">Most popular</span>
              )}
              ...
              <Link href={plan.href} className={`block text-center py-3 rounded-xl text-sm font-medium transition-colors ${plan.highlight ? "bg-[#c84b2f] text-white hover:bg-[#e05535]" : "border border-[#333] text-[#aaa] hover:border-[#555] hover:text-white"}`}>
                {plan.cta} →
              </Link>
            </div>
          ))}
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "Can I cancel anytime?", a: "Yes. Cancel from your dashboard at any time. No fees, no drama. Barış might be disappointed, but he'll understand." },
